...
const { compareAsc, parseISO, isWeekend } = require("date-fns");
...
//Modelo
const model = require("./models/mdl_updateSolicitudVacaciones");

/**
 * @class updateSolicitudVacaciones
 *
 * La clase se encarga de realizar las respectivas validaciones y transformaciones para modificar una solicitud.
 *
...
class updateSolicitudVacaciones {
    ...
    async #validations() {
        ...
        if (bitFinDeSemana) {
            throw new Error("La fecha de inicio no puede ser un fin de semana.");
        }
        ...
        if (bitFechaMayorIgual < 0) {
            throw new Error("La fecha de reingreso debe ser mayor a la fecha fin.");
        }

        if (this.#objData.intNumeroDiasCompensar > this.#objData.intNumeroDiasDisfrutar) {
            throw new Error(
    ...
    async #completeData() {
        ...
        if (this.#objData.bitOperacion) {
            ...
            arrAprobadores.push({ strEmailAprobador: objCliente.strGerenteServicio });

            if (objCliente.strGerenteServicio !== "kospinar@choucairtesting.com") {
                arrAprobadores.push({
                    strEmailAprobador: "geriras@choucairtesting.com",
                });
            }else{
                arrAprobadores.push({
                    strEmailAprobador: "geriras@choucairtesting.com",
        ...
        let newData = {
            ...
            dtFechaFin,
            dtFechaReingreso,
            intNumeroDiasCompensar: parseInt(this.#objData.intNumeroDiasCompensar),
            intNumeroDiasDisfrutar: parseInt(this.#objData.intNumeroDiasDisfrutar),
            ...
            intIdEstado: this.#objSolicitudActual.intIdEstado,
        };
    ...
    async #updateSolicitud() {
        ...
        let query = await dao.updateSolicitud(this.#objData);
    ...
    async #updateAprobadores() {
        let dao = new classInterfaceDAOSolicitudes();
        let query;

        if (typeof this.#objData.arrAprobadores === "string") {
            query = await dao.updateAprobador({
                intId: this.#objSolicitudActual.arrAprobadores[0].intId,
        ...
        if (typeof this.#objData.arrAprobadores === "object") {
            for (let i = 0; i < this.#objData.arrAprobadores.length; i++) {
                ...
                query = await dao.updateAprobador({
                    intId: this.#objSolicitudActual.arrAprobadores[i].intId,
                    intIdSolicitud: this.#objData.intId,
    ...
    async #setAuditoria() {
        ...
        let query = await dao.setAuditoriaSolicitud({
            intIdSolicitud: this.#objSolicitudActual.intId,
            ...
            strObservaciones: "El usuario ha actualizado la solicitud.",
        });
    ...
    async #sendEmail() {
        ...
        let strMsg = `
        <p>El colaborador ${objDataPersonaActiva.strNombreCompleto}, identificado con ${
            objDataPersonaActiva.strtipoIdentificacion
        } - ${
            objDataPersonaActiva.strIdentificacion
        }, ha modificado la solicitud de vacaciones con identificador #${
            this.#objSolicitudActual.intId
        }.</p>
        <br>
        ...
        <ul>
          <li>
              <b>Fecha de solicitud: </b> ${this.#objSolicitudActual.dtFechaSolicitud}
          </li>
          <li>
               <b>Fecha de inicio de las vacaciones: </b> ${this.#objData.dtFechaInicio}
          </li>
          <li>
               <b>Fecha final de las vacaciones: </b> ${this.#objData.dtFechaFin}
          </li>
          <li>
               <b>Fecha de reingreso a la compañia: </b> ${this.#objData.dtFechaReingreso}
          </li>
          <li>
               <b>Número de días a descansar: </b> ${this.#objData.intNumeroDiasDisfrutar}
          </li>
          <li>
               <b>Número de días a compensar: </b> ${this.#objData.intNumeroDiasCompensar}
          </li>
        </ul>
        ...
        if (typeof this.#objData.arrAprobadores === "object") {
            strEmailToAprobadores = this.#objData.arrAprobadores[0].strEmailAprobador;
        }
        ...
        querySendEmail = await sendEmail(
            {
                ...
                subject: `Modificación de la Solicitud de Vacaciones - ${objDataPersonaActiva.strNombreCompleto}`,
                message: htmlMsg,
            },
            process.env.TOKEN_SYSTEM
        );
    ...
    async #rollbackTransaction() {
        try {
            ...
            for (let i = 0; i < this.#objSolicitudActual.arrAprobadores.length; i++) {
                let query = await dao.updateAprobador({
                    intId: this.#objSolicitudActual.arrAprobadores[i].intId,
                    intIdSolicitud: this.#objData.intId,
                    strAprobador: this.#objSolicitudActual.arrAprobadores[i].strAprobador,
                });
            ...
            this.#objResult = {
                error: true,
                msg: "La modificación de la solicitud ha fallado, se devolvieron los cambios efectuados en el sistema, por favor contacta al área de TI para más información.",
            };
        } catch (error) {
            this.#objResult = {
                error: true,
...
module.exports = updateSolicitudVacaciones;
